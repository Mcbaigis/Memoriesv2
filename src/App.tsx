...
import { 
  Heart, Calendar, MapPin, Briefcase, Camera, Home, Music, X, ChevronRight, Sun, Baby, Users, Star, Globe, Filter, Coffee, Smile, Armchair, Book, Image as ImageIcon, Edit2, Save, ExternalLink, PlayCircle, Plus, RotateCcw, Lock, Unlock, Trash2, Check, Link as LinkIcon, AlertTriangle, ArrowUp, ZoomIn, ZoomOut, Maximize, Upload, ArrowLeft, ArrowRight, Play, Pause, Cake, Settings, LogOut, Loader2, Newspaper, StickyNote, Layers, Search
} from 'lucide-react';
...
const Modal = ({ isOpen, onClose, children, color = "bg-white", zIndex = "z-50" }) => {
...
const EditorModal = ({ isOpen, title, fields, onClose, onSave, onUpload }) => {
...
const PinModal = ({ isOpen, onClose, onSuccess, correctPin }) => {
...
const LoginScreen = ({ onLogin, correctPin }) => {
    const [pin, setPin] = useState("");
    const [error, setError] = useState(false);
    const handleSubmit = (e) => { e.preventDefault(); if (pin === correctPin) { onLogin(); } else { setError(true); } };
    return (
...
// --- TIMELINE VISUALS ---
const ScrapbookItem = ({ item, onClick }) => {
...
const TimelineZoomControls = ({ zoom, setZoom }) => {
    return (
...
const TimelineEraView = ({ events, onSelectEra }) => {
    ...
    const sortedDecades = Object.values(decades).sort((a,b) => a.year - b.year);
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {sortedDecades.map(d => (
                <button key={d.year} onClick={() => onSelectEra(d.year)} className="bg-white border-2 border-slate-100 rounded-3xl p-6 hover:border-indigo-200 hover:shadow-xl transition-all text-left group relative overflow-hidden">
                    ...
                    <div className="flex -space-x-3">{d.images.map((img, i) => (<div key={i} className="w-12 h-12 rounded-full border-2 border-white shadow-md overflow-hidden"><img src={img} className="w-full h-full object-cover" /></div>))}{d.count > 0 && <div className="w-12 h-12 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-500">View</div>}</div>
                </button>
            ))}
        </div>
    );
...
const TimelineYearView = ({ events, onSelectEvent }) => {
    return (
...
const TimelineScrapbookView = ({ events, onSelectEvent }) => {
    return <div className="max-w-2xl space-y-8">{events.map((item, index) => <ScrapbookItem key={index} item={item} onClick={onSelectEvent} />)}</div>;
}

// ... Nodes ...
const FamilyMemberNode = ({ member, onClick }) => ( <button onClick={() => onClick(member)} className="relative flex flex-col items-center group hover:-translate-y-1 transition-transform"><div className={`w-20 h-20 rounded-full ${member.color} flex items-center justify-center mb-2 shadow-lg border-2 border-white overflow-hidden`}>{member.image ? <img src={member.image} className="w-full h-full object-cover"/> : <span className="text-2xl font-bold text-white opacity-95">{member.name.charAt(0)}</span>}</div><div className="bg-white/90 px-3 py-1.5 rounded-lg shadow-sm border text-center min-w-[90px]"><h3 className="font-bold text-slate-800 text-xs">{member.name.split(' ')[0]}</h3><p className="text-indigo-500 font-medium text-[10px] uppercase">{member.relation}</p></div></button>);
const FriendCard = ({ friend, onClick }) => (<button onClick={() => onClick(friend)} className="bg-white p-6 rounded-3xl shadow-sm border hover:shadow-xl transition-all w-full text-left flex gap-5 items-center"><div className={`w-16 h-16 rounded-full ${friend.color} flex-shrink-0 flex items-center justify-center overflow-hidden`}>{friend.image ? <img src={friend.image} className="w-full h-full object-cover"/> : <span className="text-2xl font-bold text-white">{friend.name.charAt(0)}</span>}</div><div><h3 className="font-bold text-lg">{friend.name}</h3><div className="flex flex-col gap-1 text-sm"><div className="flex items-center gap-2 text-indigo-500 font-medium"><MapPin size={14} /> Met: {friend.metAt}</div></div></div><ChevronRight className="text-slate-300 group-hover:text-indigo-400 transition-colors" /></button>);
const HomeCard = ({ home, onClick }) => (<button onClick={() => onClick(home)} className="group relative w-full overflow-hidden rounded-3xl shadow-md hover:shadow-2xl transition-all aspect-[4/3]"><div className={`absolute inset-0 ${home.color}`}>{home.image ? <img src={home.image} className="w-full h-full object-cover opacity-90"/> : <Home size={80} className="text-white/30 m-auto mt-12"/>}</div><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-6 text-left"><h3 className="text-2xl font-bold text-white">{home.name}</h3><p className="text-white/80 text-sm">{home.address}</p></div></button>);
const FilterToggle = ({ label, icon, active, onClick, colorClass }) => (<button onClick={onClick} className={`flex items-center gap-2 px-4 py-2 rounded-full border-2 transition-all ${active ? `${colorClass} text-white border-transparent shadow-md` : 'bg-white text-slate-400 border-slate-200 hover:border-slate-300'}`}>{React.cloneElement(icon, { size: 16 })}<span className="font-bold text-sm">{label}</span>{active && <span className="ml-1 bg-white/20 px-1.5 rounded text-xs">✓</span>}</button>);
...
const NavButtonMobile = ({ active, onClick, icon, label }) => ( <button onClick={onClick} className={`flex flex-col items-center gap-1 p-2 min-w-[64px] ${active ? 'text-indigo-600 -translate-y-2' : 'text-slate-400'}`}><div>{React.cloneElement(icon, { size: 20 })}</div><span className="text-[10px]">{label}</span></button> );

const TreeBranch = ({ person, allMembers, onSelect, stopRecursion = false }) => {
    const spouse = allMembers.find(m => m.id === person.spouseId);
    const children = stopRecursion ? [] : allMembers.filter(m => (m.parentId === person.id || (spouse && m.parentId === spouse.id)) && m.id !== person.spouseId);
...
const FamilyTreeView = ({ familyMembers, viewRootId, setViewRootId, onSelect, zoom, scrollContainerRef, handleZoomIn, handleZoomOut, handleResetZoom, isEditMode }) => {
    ...
    const currentRoot = familyMembers.find(m => m.id === viewRootId) || familyMembers[0];
    let parents = [];
    let siblings = [];
    
    if (currentRoot) {
        if (currentRoot.parentId) {
            const directParent = familyMembers.find(m => m.id === currentRoot.parentId);
            parents = directParent ? [directParent] : [];
        }
        if (viewRootId === 'jackie') {
           siblings = familyMembers.filter(m => m.parentId === currentRoot.parentId && m.id !== currentRoot.id && m.id !== currentRoot.spouseId);
        }
    }
           ...
           <div className="min-w-[2000px] min-h-[1500px] flex flex-col items-center justify-start pt-32 origin-top-center transition-transform duration-200" style={{transform: `scale(${zoom})`}}>
              {parents.length > 0 && (
                  <div className="flex flex-col items-center mb-12">
                      <div className="flex gap-16">{parents.map(p => <TreeBranch key={p.id} person={p} allMembers={familyMembers} onSelect={onSelect} stopRecursion={true} />)}</div>
              ...
              <div className="flex items-center gap-8 mb-12">
                  ...
                  {viewRootId === 'jackie' && (<div className="flex flex-col gap-4 mr-8">{siblings.map(sib => (<button key={sib.id} onClick={() => setViewRootId(sib.id)} className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-200 hover:bg-indigo-50 hover:border-indigo-200 transition-all text-slate-600 font-medium"><span className="w-2 h-2 rounded-full bg-indigo-400"></span>See {sib.name}'s Family <ArrowRight size={14} /></button>))}</div>)}
...
export default function App() {
  ...
  const [storageRefState, setStorageRefState] = useState(null);
  ...
  const [editModalConfig, setEditModalConfig] = useState({ isOpen: false, title: '', fields: [], onSave: () => {} });
  ...
  const [familyMembers, setFamilyMembers] = useState(DEFAULT_FAMILY); 
  ...
  useEffect(() => {
      ...
      const unsubFamily = onSnapshot(collection(db, `${dataPath}/familyMembers`), (snap) => {
        ...
        // IF DB IS EMPTY OR ERROR, WE KEEP DEFAULT_FAMILY (Initialized in state)
      ...
      const unsubFriends = onSnapshot(collection(db, `${dataPath}/friends`), (snap) => { const data = snap.docs.map(d => d.data()); if (data.length > 0) setFriends(data); else if(snap.empty) DEFAULT_FRIENDS.forEach(async (m, i) => await setDoc(doc(db, `${dataPath}/friends`, String(i)), m)); }, (err) => console.log("Waiting for permissions...", err.code));
      const unsubHomes = onSnapshot(collection(db, `${dataPath}/homes`), (snap) => { const data = snap.docs.map(d => d.data()); if (data.length > 0) setHomes(data); else if(snap.empty) DEFAULT_HOMES.forEach(async (m) => await setDoc(doc(db, `${dataPath}/homes`, m.id), m)); }, (err) => console.log("Waiting for permissions...", err.code));
      const unsubJournal = onSnapshot(collection(db, `${dataPath}/journal`), (snap) => { const data = snap.docs.map(d => d.data()); if (data.length > 0) setJournalEntries(data); else if(snap.empty) DEFAULT_JOURNAL.forEach(async (m) => await setDoc(doc(db, `${dataPath}/journal`, String(m.id)), m)); }, (err) => console.log("Waiting for permissions...", err.code));
      const unsubAlbums = onSnapshot(collection(db, `${dataPath}/albums`), (snap) => { const data = snap.docs.map(d => d.data()); if (data.length > 0) setAlbums(data); else if(snap.empty) DEFAULT_ALBUMS.forEach(async (m) => await setDoc(doc(db, `${dataPath}/albums`, String(m.id)), m)); }, (err) => console.log("Waiting for permissions...", err.code));
      const unsubProfile = onSnapshot(collection(db, `${dataPath}/metadata`), (snap) => { snap.docs.forEach(d => { if (d.id === 'profile') setProfile(d.data()); if (d.id === 'quickPrompt') setQuickPrompt(d.data()); }); }, (err) => console.log("Waiting for permissions...", err.code));
  ...
  const dbUpdate = async (collectionName, id, data) => { if (!db) return; await setDoc(doc(db, `artifacts/${appId}/public/data/${collectionName}`, String(id)), data); };
  const dbDelete = async (collectionName, id) => { if (!db) return; await deleteDoc(doc(db, `artifacts/${appId}/public/data/${collectionName}`, String(id))); };
  const handleUpload = async (file) => { if (!storageRefState) return ""; const storageRef = ref(storageRefState, 'images/' + file.name + '_' + Date.now()); await uploadBytes(storageRef, file); return await getDownloadURL(storageRef); };
  ...
  const handleUpdateSettings = () => { const fields = [{ name: "siteTitle", label: "Site Title", value: appSettings.siteTitle }, { name: "familyPin", label: "Family PIN", value: appSettings.familyPin }, { name: "adminPin", label: "Admin PIN", value: appSettings.adminPin }]; setEditModalConfig({ isOpen: true, title: "Settings", fields, onSave: (data) => { dbUpdate('metadata', 'settings', data); setEditModalConfig(p => ({...p, isOpen:false})); } }); };
  ...
  const sortedTimeline = allTimelineEvents.filter(item => filters[item.category]).filter(item => item.year >= startYear && item.year <= endYear).sort((a, b) => a.year - b.year);
  const filteredJournal = journalEntries.sort((a, b) => new Date(b.date) - new Date(a.date));
  ...
  const handleAddEvent = () => { 
      ...
      setEditModalConfig({ isOpen: true, title: "Add Event", fields, onSave: (data) => {
          const id = Date.now();
  ...
  const handleEditProfile = (e) => { e.stopPropagation(); setEditModalConfig({isOpen: true, title: "Edit Profile", fields: [{ name: "name", label: "Name", value: profile.name }, { name: "details", label: "Details", value: profile.details }], onSave: (data) => { dbUpdate('metadata', 'profile', data); setEditModalConfig(p => ({...p, isOpen:false})); }}); };
  const handleEditPrompt = (e) => { e.stopPropagation(); setEditModalConfig({isOpen: true, title: "Edit Reminder", fields: [{ name: "text", label: "Text", value: quickPrompt.text, type: "textarea" }], onSave: (data) => { dbUpdate('metadata', 'quickPrompt', data); setEditModalConfig(p => ({...p, isOpen:false})); }}); };
  const handleEditEvent = (e) => { e.stopPropagation(); const fields = [{ name: "title", label: "Title", value: selectedEvent.title }, { name: "year", label: "Year", value: selectedEvent.year }, { name: "description", label: "Desc", value: selectedEvent.description, type: "textarea" }, { name: "location", label: "Loc", value: selectedEvent.location }, { name: "type", label: "Style", value: selectedEvent.type, type: "select", options: [{value: 'memory', label: 'Standard Card'}, {value: 'headline', label: 'Headline Style'}, {value: 'photo', label: 'Polaroid Photo'}] }, { name: "image", label: "Image", value: selectedEvent.image, type: "file" }]; setEditModalConfig({ isOpen: true, title: "Edit Event", fields, onSave: (data) => { const styles = getCategoryStyles(data.category || selectedEvent.category); const updated = { ...selectedEvent, ...data, ...styles }; dbUpdate('timelineEvents', selectedEvent.id, updated); setSelectedEvent(updated); setEditModalConfig(p => ({...p, isOpen:false})); }, onUpload: handleUpload }); };
  const handleDeleteEvent = (e) => { e.stopPropagation(); setConfirmModalConfig({isOpen: true, title: "Delete", message: "Delete event?", onConfirm: () => { dbDelete('timelineEvents', selectedEvent.id); setSelectedEvent(null); setConfirmModalConfig(p => ({...p, isOpen:false})); }}); };
  
  // MEMBER EDIT: Only allow edits, no add/delete for family structure
  const handleEditMember = (e) => { e.stopPropagation(); const parents = familyMembers.filter(m => m.id !== selectedPerson.id).map(m => ({ value: m.id, label: m.name })); const fields = [{ name: "name", label: "Name", value: selectedPerson.name }, { name: "relation", label: "Relation", value: selectedPerson.relation }, { name: "dob", label: "DOB", value: selectedPerson.dob }, { name: "image", label: "Photo", value: selectedPerson.image, type: "file" }, { name: "details", label: "Details", value: selectedPerson.details, type: "textarea" }]; setEditModalConfig({ isOpen: true, title: "Edit Member", fields, onSave: (data) => { const updated = { ...selectedPerson, ...data }; dbUpdate('familyMembers', selectedPerson.id, updated); setSelectedPerson(updated); setEditModalConfig(p => ({...p, isOpen:false})); }, onUpload: handleUpload }); };
  // const handleDeleteMember = () => { setConfirmModalConfig({ isOpen: true, title: "Delete", message: "Remove person?", onConfirm: () => { dbDelete('familyMembers', selectedPerson.id); setSelectedPerson(null); setConfirmModalConfig(p => ({...p, isOpen:false})); }}); };
  ...
  const handleEditPerson = (e) => { e.stopPropagation(); if (selectedPerson.hasOwnProperty('generation')) { handleEditMember(e); } else { const fields = [{ name: "name", label: "Name", value: selectedPerson.name }, { name: "metAt", label: "Met At", value: selectedPerson.metAt }, { name: "frequency", label: "Freq", value: selectedPerson.frequency }, { name: "image", label: "Photo", value: selectedPerson.image, type: "file" }, { name: "details", label: "Details", value: selectedPerson.details, type: "textarea" }]; setEditModalConfig({ isOpen: true, title: "Edit Friend", fields, onSave: (data) => { const updated = { ...selectedPerson, ...data }; dbUpdate('friends', selectedPerson.id, updated); setSelectedPerson(updated); setEditModalConfig(p => ({...p, isOpen:false})); }, onUpload: handleUpload }); } };
  const handleDeletePersonAny = () => { if (selectedPerson.hasOwnProperty('generation')) { alert("Family structure is fixed."); } else { setConfirmModalConfig({ isOpen: true, title: "Delete", message: "Remove friend?", onConfirm: () => { dbDelete('friends', selectedPerson.id); setSelectedPerson(null); setConfirmModalConfig(p => ({...p, isOpen:false})); }}); } };
  const handleAddHome = () => { const fields = [{ name: "name", label: "Name", value: "" }, { name: "address", label: "Address", value: "" }, { name: "years", label: "Years", value: "" }, { name: "image", label: "Photo", value: "", type: "file" }]; setEditModalConfig({ isOpen: true, title: "Add Home", fields, onSave: (data) => { const id = Date.now(); const newHome = { id, color: "bg-indigo-500", description: "...", features: [], rooms: [], ...data }; dbUpdate('homes', id, newHome); setEditModalConfig(p => ({...p, isOpen:false})); }, onUpload: handleUpload }); };
  const handleEditHome = () => { const fields = [{ name: "name", label: "Name", value: selectedHome.name }, { name: "address", label: "Address", value: selectedHome.address }, { name: "years", label: "Years", value: selectedHome.years }, { name: "description", label: "Description", value: selectedHome.description, type: "textarea" }, { name: "image", label: "Photo", value: selectedHome.image, type: "file" }]; setEditModalConfig({ isOpen: true, title: "Edit Home", fields, onSave: (data) => { const updatedHome = { ...selectedHome, ...data }; dbUpdate('homes', selectedHome.id, updatedHome); setSelectedHome(updatedHome); setEditModalConfig(p => ({...p, isOpen:false})); }, onUpload: handleUpload }); };
  const handleAddRoom = () => { const fields = [{name: "name", label: "Room Name", value: ""}, {name: "desc", label: "Desc", value: "", type: "textarea"}, {name: "image", label: "Photo", value: "", type: "file"}]; setEditModalConfig({ isOpen: true, title: "Add Room", fields, onSave: (data) => { const newRoom = { name: data.name, desc: data.desc, image: data.image, color: "bg-indigo-100" }; const updatedHome = { ...selectedHome, rooms: [...(selectedHome.rooms || []), newRoom] }; dbUpdate('homes', selectedHome.id, updatedHome); setSelectedHome(updatedHome); setEditModalConfig(p => ({...p, isOpen:false})); }, onUpload: handleUpload }); };
  const handleDeleteRoom = (index) => { setConfirmModalConfig({ isOpen: true, title: "Delete Room", message: "Delete?", onConfirm: () => { const updatedRooms = selectedHome.rooms.filter((_, i) => i !== index); const updatedHome = { ...selectedHome, rooms: updatedRooms }; dbUpdate('homes', selectedHome.id, updatedHome); setSelectedHome(updatedHome); setConfirmModalConfig(p => ({...p, isOpen:false})); }}); };
  ...
  const handleEditAlbum = (e, id) => {
    ...
    setEditModalConfig({
        ...
        onSave: (data) => {
            ...
            setEditModalConfig(p => ({...p, isOpen:false}));
        },
        onUpload: handleUpload
    });
  };
  const handleDeleteAlbum = (e, id) => { e.stopPropagation(); e.preventDefault(); setConfirmModalConfig({ isOpen: true, title: "Delete", message: "Delete?", onConfirm: () => { dbDelete('albums', id); setConfirmModalConfig(p => ({...p, isOpen:false})); }}); };
  const saveEditedEntry = (id) => { const entry = journalEntries.find(e => e.id === id); const updated = { ...entry, content: editContent }; dbUpdate('journal', id, updated); setEditingEntryId(null); };
  ...
  const deleteEntry = (id) => { if(window.confirm("Delete?")) dbDelete('journal', id); };
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-24 md:pb-0 md:pl-20">
      <EditorModal isOpen={editModalConfig.isOpen} title={editModalConfig.title} fields={editModalConfig.fields} onSave={editModalConfig.onSave} onClose={() => setEditModalConfig(prev => ({...prev, isOpen: false}))} onUpload={handleUpload} />
      <ConfirmModal isOpen={confirmModalConfig.isOpen} title={confirmModalConfig.title} message={confirmModalConfig.message} onConfirm={confirmModalConfig.onConfirm} onClose={() => setConfirmModalConfig(prev => ({...prev, isOpen: false}))} />
        ...
        {activeTab === 'friends' && (
             <div>
                <header className="mb-10 flex justify-between">
                    ...
                    {isEditMode && <button onClick={handleAddFriend} className="bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2"><Plus size={18} /> Add</button>}
                </header>
        ...
        {activeTab === 'houses' && (
            <div>
                <header className="mb-10 flex justify-between">
                    ...
                    {isEditMode && <button onClick={handleAddHome} className="bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2"><Plus size={18} /> Add</button>}
                </header>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className={`h-32 ${selectedPerson.color} relative`}>
                     {isEditMode && (
                         <div className="absolute top-4 right-4 flex gap-2">
                             <button onClick={handleEditPerson} className="bg-white/20 p-2 rounded-full text-white hover:bg-white/40"><Edit2 size={18}/></button>
                         </div>
                     )}
